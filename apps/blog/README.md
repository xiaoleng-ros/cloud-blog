...
基于 Astro 的个人博客前端，是 `cloud-blog` monorepo 的一部分。本目录是**站点前端应用**（Astro，独立依赖、独立部署），内容全部来自后台 CMS；后台位于仓库根 `../cms`，完整说明见根目录 `README.md`。

包含文章、随笔、归档、分类、标签、搜索、RSS、站点地图、Twikoo 后端评论与关于页等页面。

## 内容来源

前台不再维护本地 Markdown / JSON，内容一律从后台（Payload CMS）拉取：
...
- **站点信息（名称 / 描述 / 作者）与 Hero 文案**：优先读取后台 `SiteSettings` 全局，后台缺失时回退到 `src/lib/site-defaults.ts` 的默认值；导航菜单来自后台 `Navigation` 全局。
...
后台未运行时，构建/开发仍可进行，只是文章/随笔/项目内容为空，页面仍可正常访问（站点默认值兜底）。
...
│   ├── layouts/           # 页面布局（BaseLayout 内含运行时同步脚本）
│   ├── lib/               # 后台取数（payload-api / payload-loader）、站点默认值、Markdown 插件
...
2. 在平台环境变量中设置 `SITE_URL` 为正式域名，并设置 `PUBLIC_PAYLOAD_URL` 指向后台地址（构建时拉取内容用）
